...
export default function TaskModal({ task, boardMembers, onClose }: Props) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
            ...
            <div
                className="relative bg-dark-900 border border-dark-700 rounded-2xl w-full max-w-lg shadow-2xl animate-scale-in max-h-[90vh] overflow-y-auto"
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-3 bg-dark-800 border border-dark-600 rounded-xl text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition-all"
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-3 bg-dark-800 border border-dark-600 rounded-xl text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition-all [color-scheme:dark]"
                        />
                    </div>

                    {/* Assignees */}
                    <div>
                        <label className="block text-sm font-medium text-dark-300 mb-2">Assign Members</label>
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${isAssigned
                                            ...
                                            : 'bg-dark-800 border border-transparent hover:bg-dark-700'
                                            }`}
                                    >
                                        <div className="w-7 h-7 bg-gradient-to-br from-primary-500 to-primary-700 rounded-full flex items-center justify-center text-xs font-medium text-white">
                                            {member.user.name.charAt(0).toUpperCase()}
                                        </div>
                                        <span className="text-sm text-white flex-1 text-left">{member.user.name}</span>
                                        {isAssigned && (
